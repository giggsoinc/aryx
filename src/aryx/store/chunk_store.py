...
class ChunkStore:
    ...
    def upsert_document(
        ...
        with self._conn.cursor() as cur:
            cur.execute(load("upsert_document"), (content_hash, file_name, source_type, byte_count))
            row = cur.fetchone()
        self._conn.commit()
    ...
    def save_embeddings(self, chunk_db_ids: list[int], embeddings: list[ChunkEmbedding]) -> None:
        with self._conn.cursor() as cur:

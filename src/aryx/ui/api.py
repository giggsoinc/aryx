...
def _delete(path: str) -> Any:
    """Issue a DELETE request to a path and decode the JSON reply."""
...
def search_entities(name: str = "", etype: str = "") -> list[dict]:
    """Search entities by name and/or ontology type."""
    params = []
    if name:
...
def get_neighbors(entity_id: int) -> list[dict]:
    """Return the immediate neighbours of an entity."""
    return _get(f"/entities/{entity_id}/neighbors")


def get_provenance(entity_id: int) -> list[dict]:
    """Return the source records that resolved into an entity."""
...
def list_runs() -> list[dict]:
    """Return discovery/ingest run records."""
    return _get("/admin/runs")


def list_jobs() -> list[dict]:
    """Return background job records."""
    return _get("/admin/jobs")


def get_job(job_id: str) -> dict[str, Any]:
    """Return a single job's status and progress."""
...
def get_llm_config() -> dict[str, Any]:
    """Return the active LLM provider/model config."""
    return _get("/llm/config")


def set_llm_config(cfg: dict) -> dict[str, Any]:
    """Update the LLM provider/model config (live, no restart)."""
    return _post("/admin/llm/config", cfg)


def observability() -> dict[str, Any]:
    """Return job/token/graph observability metrics."""

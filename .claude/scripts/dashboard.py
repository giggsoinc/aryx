...
def audit_drift(metrics: dict, metadata: dict, sample_rate: float = 0.01) -> dict:
    ...
    # Write audit log
    audit_dir = PROJECT_DIR / ".raven" / "audit"
...
def main():
    ...
    parser.add_argument("--current-project", action="store_true",
                        help="Auto-filter by project name read from .raven/manifest.json")
    args = parser.parse_args()

    # --current-project: read project name from .raven/manifest.json in CWD
    if args.current_project and not args.project:
        try:
            manifest = PROJECT_DIR / ".raven" / "manifest.json"
            args.project = json.loads(manifest.read_text()).get("project")
        except Exception:
            pass
        ...
        if args.open:
            try:
                ...
                if platform.system() == "Windows":
                    os.startfile(str(VAULT_DASHBOARD_HTML))
                elif platform.system() == "Darwin":
                    subprocess.run(["open", str(VAULT_DASHBOARD_HTML)], check=False)
                else:
                    subprocess.run(["xdg-open", str(VAULT_DASHBOARD_HTML)], check=False)
            except Exception:
                pass

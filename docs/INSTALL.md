...
Get **Aryx Lite 1.9.0** running with Docker Compose (recommended), then open the web UI and onboard data.
...
- `ARYX_IMAGE` (default `giggsodocker/aryx-lite:1.9.0`)
- `ARYX_WEB_IMAGE` (default `giggsodocker/aryx-lite-web:1.9.0`)

### Tags (both images)

| Tag | Meaning |
|-----|---------|
| `1.9.0` | Semver from `pyproject.toml` / `aryx.__version__` — always pull an explicit version |
| `v1.9.0` | Same release, `v`-prefixed |
| `<git-sha>` | Exact commit (e.g. `a98a954`) |

```bash
# Pull both images explicitly
docker pull giggsodocker/aryx-lite:1.9.0
docker pull giggsodocker/aryx-lite-web:1.9.0
...
# Pin a version
export ARYX_IMAGE=giggsodocker/aryx-lite:1.9.0
export ARYX_WEB_IMAGE=giggsodocker/aryx-lite-web:1.9.0
docker compose up -d
```

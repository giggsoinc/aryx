...
Get **Aryx Lite 1.6.1** running with Docker Compose (recommended), then open the web UI and onboard data.
...
- `ARYX_IMAGE` (default `giggsodocker/aryx-lite:1.6.1`)
- `ARYX_WEB_IMAGE` (default `giggsodocker/aryx-lite-web:1.6.1`)

### Tags (both images)

| Tag | Meaning |
|-----|---------|
| `1.6.1` | Semver from `pyproject.toml` / `aryx.__version__` — always pull an explicit version |
| `v1.6.1` | Same release, `v`-prefixed |
...
# Pin a version
export ARYX_IMAGE=giggsodocker/aryx-lite:1.6.1
export ARYX_WEB_IMAGE=giggsodocker/aryx-lite-web:1.6.1
docker compose up -d
```
